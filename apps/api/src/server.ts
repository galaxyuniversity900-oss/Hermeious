...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/capabilities/execute') {
    try {
      ...
      const body = JSON.parse(Buffer.concat(chunks).toString() || '{}') as { capability?: string; input?: Record<string, unknown>; approved?: boolean };
      if (!body.capability) return json(res, 400, { error: 'capability is required' });
      const decision = policy.check(registry, body.capability, body.approved === true);
      if (!decision.allowed) return json(res, 403, { error: decision.reason });
      const result = await registry.execute(body.capability, body.input ?? {}, { requestId: randomUUID(), approved: body.approved === true });
